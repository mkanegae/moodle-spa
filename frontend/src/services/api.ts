...
class MoodleAPI {
  ...
  async getCourses(): Promise<Course[]> {
    try {
      console.log('Making request for courses with token:', this.token?.substring(0, 10) + '...');
  ...
  async getResourceHtmlContents(courseid: number): Promise<any[]> {
    try {
      ...
      console.log('Fetching resource HTML contents for course:', courseid);
      ...
      for (const section of sections) {
        ...
        for (const module of section.modules) {
          ...
          console.log('Found resource module:', module.name);

          // コンテンツファイルを確認
          if (module.contents && Array.isArray(module.contents)) {
            for (const content of module.contents) {
              console.log('Content file:', content.filename, 'mimetype:', content.mimetype);

              // HTMLファイルのみを対象
              if (content.mimetype === 'text/html' || content.mimetype === 'application/xhtml+xml') {
                try {
                  // MoodleのファイルURLにトークンを追加
                  let fileUrl = content.fileurl;
                  if (this.token && !fileUrl.includes('token=')) {
                    const separator = fileUrl.includes('?') ? '&' : '?';
                    fileUrl = `${fileUrl}${separator}token=${this.token}`;
                  }

                  console.log('Fetching HTML from:', fileUrl);
                  ...
                  console.log('HTML content retrieved, length:', response.data.length);
                  ...
                  console.error(`Failed to fetch HTML content from ${content.fileurl}:`, error.message);
                }
              }
            }
          }
      ...
      console.log('Total HTML contents found:', htmlContents.length);
      return htmlContents;
    } catch (error) {
  ...
  async fetchMarkdownFile(fileUrl: string): Promise<string> {
    try {
      // MoodleのファイルURLにトークンを追加
      let urlWithToken = fileUrl;
      if (this.token && !fileUrl.includes('token=')) {
        const separator = fileUrl.includes('?') ? '&' : '?';
        urlWithToken = `${fileUrl}${separator}token=${this.token}`;
      }

      console.log('Fetching Markdown from:', urlWithToken);
      ...
      console.log('Markdown content retrieved, length:', response.data.length);
      return response.data;
    } catch (error: any) {
  ...
  addTokenToFileUrl(fileUrl: string): string {
    // MoodleのファイルURLにトークンを追加
    if (!this.token || fileUrl.includes('token=')) {
      return fileUrl;
    }

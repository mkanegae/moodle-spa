...
@app.get(
    "/api/profile/{userid}",
    response_model=dict,
    summary="プロフィール情報取得（エイリアス）",
    tags=["User Profile"]
)
def get_profile_alias(userid: int, db: Session = Depends(get_db)):
    """
    プロフィール情報取得のエイリアスエンドポイント
    /api/users/{userid}/profile と同じ
    """
...
@app.post(
    ...
    response_model=dict,
    summary="プロフィール情報更新（エイリアス）",
    tags=["User Profile"]
)
def update_profile_alias(userid: int, data: ProfileSettingsUpdate, db: Session = Depends(get_db)):
    """
    プロフィール情報更新のエイリアスエンドポイント
    /api/users/{userid}/profile POST と同じ
    """
    return update_user_profile(userid, data, db)
...
def get_resume_course_alias(userid: int, limit: int = 5, db: Session = Depends(get_db)):
    ...
    /api/users/{userid}/resume-courses と同じ
    """
    return get_resume_courses(userid, limit, db)
